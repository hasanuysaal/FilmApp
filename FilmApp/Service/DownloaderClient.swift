...
class DownloaderClient : ObservableObject {
    ...
    func downloadFilmDetails(imdbId : String, completion: @escaping (Result<FilmDetail, DownloaderError>)-> Void){
        
        guard let url = URL(string: "https://www.omdbapi.com/?i=\(imdbId)&apikey=ecc6037a") else {
            return completion(.failure(.wrongUrl))
        }
